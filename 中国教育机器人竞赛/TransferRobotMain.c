			...
			while(1)
			{
				...
				if(ultrasonic()<3/*&&ultrasonic()>=0*/)
				{
					if(Counter != 0/*||Counter != 3*/)	//当不是A点和E点的时候
					{
						...
						RotationAngle(-180);//machine(2,132,132);后转//后转
						calibration(10);
						flag_1=0;
						break;					
...
//------------------------------------------------通过循迹回道黑线交叉的地方方向指向出发区放物块的黑线上	  	
		...
		while(1)
		{
			...
			{
				go_forward(40);	//红外线检测到十字路口后再前进一点保证在十字路口的中心位置
				calibration(10);
				Counter++;		 
   ...
   else	//-------------------------如果不是黑色，就把物块送到黑线交叉处，进行判断
   {
		...
		Counter++;		  //Counter为0时为A,1为B，2为C，3为D，4为E
	 	RotationAngle(45);	//旋转到45度以后指定到B点
		calibration(10);	 
 ...
 void Return_B(void)
 {
   ...
   if(COLOUR==COLOUR_WHITE)//-------------------------如果是白色
   {
		...
		while(1)
		{
			...
			{
				go_forward(50);	//红外线检测到十字路口后再前进一点保证在十字路口的中心位置
				calibration(10);	 
		...
		while(1)
		{
			...
			{
				go_forward(55);	//红外线检测到十字路口后再前进一点保证在十字路口的中心位置
				calibration(10);	 
 ...
 ***************************************************************************************************/
 void Return_C(void)
 {
   ...
   if(COLOUR==COLOUR_RED)//-------------------------如果是红色
   {
		...
		go_forward(30);	
		RotationAngle(45); RotationAngle(90); //machine(2,132,132);//旋转指向D点
		Counter++;		  //Counter为0时为A,1为B，2为C，3为D，4为E
		calibration(10);
		...
		while(1)//--------------------------通过循迹回到黑线交叉的地方，然后转向红色区域C点
		{
			...
			{
				go_forward(63);	//红外线检测到十字路口后再前进一点保证在十字路口的中心位置
				calibration(10);	 
		 ...
		 if(COLOUR!=COLOUR_WHITE&&COLOUR!=COLOUR_YELLOW)
		 {
			...
			while(1)//--------------------------通过循迹回到黑线交叉的地方，然后转向红色区域C点
			{
			 ...
			 WedgesAddressChoice(COLOUR);//从黑线交叉的地方前往指定颜色的区域
		 	 ...
		 	 RotationAngle(135);//RotationAngle(45);//旋转135度，指向D点
			 Counter++;		  //Counter为0时为A,1为B，2为C，3为D，4为E
 ...
 void Return_D(void)
 {
   ...
   if(COLOUR==COLOUR_BLACK)//-------------------------如果是黑色
   {
		...
		while(1)//--------------------------通过循迹回到黑线交叉的地方，然后转向红色区域C点
		{
			...
			{
				go_forward(60);	//红外线检测到十字路口后再前进一点保证在十字路口的中心位置
				calibration(10);	 
				break;
			}
		 }
...
//			go_forward(3);
	...
	while(1)
	{
		...
		else
		{
			...
			go_forward(62);	//--------------交叉路口太多黑线，到这里需要先前进一定的距离后再循迹
			calibration(10);	 
			break;
		}
	}
...
//			go_forward(2);
	...
	while(1)//------------------------------通过循迹回道黑线交叉的地方方向指向出发区放物块的黑线上
	{
		...
		{
			go_forward(62);	//--------------红外线检测到十字路口后再前进一点保证在十字路口的中心位置
			calibration(10);	 
			RotationAngle(-45);	//----------旋转45度后。再前进一点
//			go_forward(5);
			calibration(20);	 
...
//------------------------------通过循迹回道黑线交叉的地方方向指向出发区放物块的黑线上	 
	...
	while(1)
	{
		...
		else
		{
			...
			go_forward(65);	//---------------红外线检测到十字路口后再前进一点保证在十字路口的中心位置
			calibration(10);	 
			RotationAngle(45);
//			go_forward(5);
			calibration(10);	 
...
//	LeftGetBlackLine();//--------------当旋转的角度不够的时候，同过向左微转来寻找黑线
	...
	while(Left_1_led==1&&Centre_1_led==1&&Right_1_led==1)
	{
		if(Flag_Rotate<15)
		{
			 machine(1,146,146); //都检测不到的时候，源地旋转
			 calibration(2);
			 Flag_Rotate++;
		}
		else{
			 machine(1,158,158); //都检测不到的时候，源地旋转
			 calibration(2);
			 Flag_Rotate++;		
		}
	}
	...
	while(1)//-----------------------------------把物块放到有颜色的区域
	{
		...
		else
		{
			if(COLOUR==COLOUR_BLUE)
				go_forward(15);	 //再前进一定的距离，把黑色物块放在黑色区域中
			calibration(1);	 
			break;
		}
	}
